...
const initialSession = {
  ...
  beardLength:'Clean',beardDensity:'Natural',cheek:'Natural',beardShape:'Rounded'
};
...
const coreNav=[
  ...
  ['/clients',UsersRound,'Clients'],['/saved',Bookmark,'Saved looks'],['/compare',Layers3,'Compare']
];
...
function CurrentVisual({result=false}) {
  ...
  return <div className={'current-visual '+(result?'result-visual':'')}>
    {session.photo&&!result?<img src={session.photo} alt="Client portrait"/>:<Sprite kind={session.clientType==='male'?'men':'women'} pos={hair.pos}/>}
    {result&&session.photo&&<div className="result-inset"><img src={session.photo} alt="Original client"/><small>ORIGINAL</small></div>}
...
function Dashboard(){
  ...
  return <Shell flush><section className="dashboard-hero"><div><p className="eyebrow light">Professional consultation platform</p><h1>Turn consideration<br/>into <em>confidence.</em></h1><p>Build, visualise and compare personalised hair directions with every client—before the first cut or colour service begins.</p><div><Button to="/consultation/client">Start consultation <ArrowRight size={17}/></Button><Button to="/clients" variant="ghost-light">Open client book</Button></div><div className="trust"><span><ShieldCheck/>Private client workflow</span><span><CheckCircle2/>Salon-ready presentation</span></div></div><div className="hero-collage"><Sprite kind="women" pos="50% 0%"/><Sprite kind="men" pos="100% 0%"/><div className="floating-card"><small>CURRENT DIRECTION</small><strong>{hair.name}</strong><span>{session.color} · {session.texture}</span></div></div></section>
  ...
  <div className="section-title"><div><p className="eyebrow">Consultation tools</p><h2>A complete styling conversation</h2></div></div><div className="tool-grid"><Link to="/consultation/client"><span>01</span><WandSparkles/><h3>New consultation</h3><p>Build a complete look through a guided, client-friendly journey.</p><b>Begin <ArrowRight/></b></Link><Link to="/styles/hair"><span>02</span><Scissors/><h3>Style library</h3><p>Browse separate professional collections for women and men.</p><b>Explore <ArrowRight/></b></Link><Link to="/compare"><span>03</span><Layers3/><h3>Side-by-side compare</h3><p>Help clients decide with up to four saved directions.</p><b>Compare <ArrowRight/></b></Link></div></section>
  </Shell>;
}
...
function ClientStep(){
  ...
  return <Shell><Workflow current="client"/><div className="consult-card"><PageHead eyebrow="Step 1 · Client profile" title="Who are we styling?" copy="This choice tailors every subsequent module and removes irrelevant options."/>
    <div className="client-types"><button className={session.clientType==='female'?'active':''} onClick={()=>chooseType('female')}><Sprite kind="women" pos="0% 0%"/><span><i>{session.clientType==='female'?<Check/>:<CircleUserRound/>}</i><strong>Female styling</strong><small>Haircut, texture and colour consultation</small></span></button><button className={session.clientType==='male'?'active':''} onClick={()=>chooseType('male')}><Sprite kind="men" pos="0% 0%"/><span><i>{session.clientType==='male'?<Check/>:<CircleUserRound/>}</i><strong>Male styling</strong><small>Haircut, grooming and colour consultation</small></span></button></div>
...
function ColorLab(){
  ...
  return <Shell><Workflow current="color"/><PageHead eyebrow={`Step ${session.clientType==='male'?'5':'4'} · Colour atelier`} title="Build the colour story" copy="Start with the foundation, add dimension, then balance the undertone."/>
    <div className="color-layout"><div className="color-preview"><CurrentVisual/><div><small>SELECTED DIRECTION</small><h2>{session.color}</h2><p>{session.technique} · {session.warmth<45?'Cool':session.warmth>55?'Warm':'Neutral'} undertone</p></div></div><div className="color-controls">
      ...
      <section><StepTitle n="03" title="Balance the warmth" copy="Fine-tune the final undertone."/><div className="warmth"><span>Cool</span><input type="range" value={session.warmth} onChange={e=>update('warmth',+e.target.value)}/><span>Warm</span></div></section>
    </div></div>
...
function Review(){
  ...
  return <Shell><Workflow current="review"/><PageHead eyebrow="Final review" title="Ready to create the look" copy="Confirm every direction before the preview enters the generation pipeline."/>
    <div className="review-layout"><div className="review-visual"><CurrentVisual/><span>REFERENCE PREVIEW</span></div><div className="review-summary"><h2>Consultation summary</h2>{rows.map(([a,b,to])=><div key={a}><span>{a}</span><strong>{b}</strong><Link to={to}>Edit</Link></div>)}<div className="provider-note"><Sparkles/><p><strong>Salon concept engine</strong><span>Generates a high-fidelity visual direction from the selected style configuration.</span></p></div><Button to="/generate" variant="full">Generate client preview <Sparkles/></Button><small className="consent"><input type="checkbox" defaultChecked/> Client consent confirmed for this consultation.</small></div></div>
...
export default function App(){
  return <Provider><Routes>
    ...
    <Route path="/studio" element={<Navigate to="/consultation/client" replace/>}/><Route path="/styles/hair" element={<HairLibrary/>}/><Route path="/styles/beard" element={<BeardLibrary/>}/>
